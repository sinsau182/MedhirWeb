      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Total requests awaiting action: {isLoading ? "..." : counts?.total || 0}
          </p>
        </div>
        <div className="bg-[#FDF7E8] text-[#E6A339] font-bold py-3 px-5 rounded-lg flex flex-col items-center shadow-sm">
          <span className="text-2xl">{isLoading ? "..." : counts?.total || 0}</span>
          <span className="text-xs mt-1">Pending Approvals</span>
        </div>
      </div>

      {isLoading ? (
        <div className="text-sm text-gray-500 py-2">Loading request statistics...</div>
      ) : error ? (
        ...
        <div className="flex flex-wrap gap-4 mt-4">
          ...
          <RequestBadge color="amber" count={counts?.profile || 0} label="Profile" />
          <RequestBadge color="green" count={counts?.expense || 0} label="Expense" />
          <RequestBadge color="purple" count={counts?.advance || 0} label="Advance" />
        </div>
      )}
...
const RequestBadge = ({ color, count, label }) => (
  <div className={`flex items-center gap-2 bg-${color}-50 px-3 py-2 rounded-lg`}>
