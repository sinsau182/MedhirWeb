...
const Leaves = () => {
  ...
  const { leaves, loading, error } = useSelector((state) => state.leaveReducer); // Fetch leaves from Redux store

  useEffect(() => {
    dispatch(fetchLeaves()); // Fetch leaves when the component mounts
  }, [dispatch]);

  console.log("Leaves:", leaves); // Log the fetched leaves
